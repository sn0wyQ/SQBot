...
Json Bot::Request(const std::string& method,
  ...
  if (response.at("ok") == false) {
    std::string description =
        "Telegram API response has field 'ok' set to false. ";
    if (response.contains("error")) {
      description += "Error: " + response.at("error").get<std::string>();
    } else {
      description += "Also response doesn't contain field 'error'";
    }

    throw Exception(ExceptionType::kRequestError, description);
  }

  if (!response.contains("result")) {
    throw Exception(ExceptionType::kRequestError,
                    "Telegram API response doesn't contain field 'result'");
  }
